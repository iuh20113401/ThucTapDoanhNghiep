...
export const initialState = {
  loading: false,
  error: null,
  shippingCity: JSON.parse(localStorage.getItem("shippingCity")) ?? [],
  cartItems: JSON.parse(localStorage.getItem("cartItems")) ?? [],
  shipping: JSON.parse(localStorage.getItem("shipping")) ?? Number(30000),
...
export const cartSlice = createSlice({
  ...
  reducers: {
    ...
    setShippingCosts: (state, { payload }) => {
      const { city, district, shippingFee } = payload;
      state.shippingCity = { city, district };
      state.shipping = shippingFee;
      localStorage.setItem("shippingCity", JSON.stringify({ city, district }));
